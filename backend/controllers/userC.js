...
const loginController = async (req, res) => {
  try {
    console.log("Login attempt with email:", req.body.email); // Log email
    ...
    const token = jwt.sign({ id: user._id }, process.env.JWT_KEY || "default_secret_key", {
      expiresIn: "1d",
    });
    user.password = undefined;
    console.log("Token generated successfully:", token); // Log token generation
...
// Enhanced docController with error handling for 403 issue
const docController = async (req, res) => {
  try {
    const { doctor, userId } = req.body;

    if (!doctor || !userId) {
      return res.status(400).send({ message: "Missing doctor or userId data", success: false });
    }
